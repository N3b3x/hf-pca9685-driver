...
class ServoController {
public:
    ServoController(PCA9685Driver* driver)
        : driver_(driver) {
    ...
    void SetTargetNormalized(uint8_t ch, float norm) {
        ...
        target_ticks_[ch] = static_cast<uint16_t>(
            SERVO_MIN_TICKS + norm * SERVO_RANGE_TICKS + 0.5f);
    }
    ...
    bool AllAtTarget() const {
        for (uint8_t i = 0; i < NUM_SERVOS; ++i) {
            if (current_ticks_[i] != target_ticks_[i]) return false;
        }
        return true;
    }
    ...
    bool Update() {
        ...
        for (uint8_t ch = 0; ch < NUM_SERVOS; ++ch) {
            int16_t delta = static_cast<int16_t>(target_ticks_[ch])
                          - static_cast<int16_t>(current_ticks_[ch]);
            if (delta == 0) continue;
            ...
            current_ticks_[ch] = static_cast<uint16_t>(
                static_cast<int16_t>(current_ticks_[ch]) + delta);
...
private:
    ...
    static uint16_t clampTicks(uint16_t t) {
        if (t < SERVO_MIN_TICKS)  return SERVO_MIN_TICKS;
        if (t > SERVO_MAX_TICKS)  return SERVO_MAX_TICKS;
        return t;
    }
};
 ...
 */
static bool ramp_to_target(ServoController& ctrl, const char* label,
 ...
 */
static void anim_wave_targets(ServoController& ctrl, uint32_t elapsed_ms,
                              uint32_t /*total_ms*/) {
 ...
 */
static void anim_mirror_targets(ServoController& ctrl, uint32_t elapsed_ms,
                                uint32_t /*total_ms*/) {
    float time_s = static_cast<float>(elapsed_ms) / 1000.0f;
 ...
 */
static void anim_knight_targets(ServoController& ctrl, uint32_t elapsed_ms,
                                uint32_t /*total_ms*/) {
    float time_s = static_cast<float>(elapsed_ms) / 1000.0f;
 ...
 */
static void anim_walk_targets(ServoController& ctrl, uint32_t elapsed_ms,
                              uint32_t /*total_ms*/) {
    float time_s = static_cast<float>(elapsed_ms) / 1000.0f;
...
static bool init_hardware() {
    ...
    ESP_LOGI(TAG, "PCA9685 initialized: 50 Hz, totem-pole, address 0x%02X",
             PCA9685_I2C_ADDRESS);
    return true;
}
...
extern "C" void app_main() {
    ...
    ESP_LOGI(TAG, "  ✅ All servos at center (1500 µs).  "
             "Position tracked: %d µs", ctrl.GetCurrentUs(0));
    ...
    while (true) {
        ...
        for (size_t i = 0; i < NUM_ANIMATIONS; ++i) {
            ...
            ESP_LOGI(TAG, "");
            ESP_LOGI(TAG, "  ┌── [%d/%d] %s  (%lu s)", (int)(i + 1), (int)NUM_ANIMATIONS,
                     anim.name, (unsigned long)(anim.duration_ms / 1000));
        ...
        ESP_LOGI(TAG, "");
        ESP_LOGI(TAG, "  All %d animations completed.  Restarting in 2 s...",
                 (int)NUM_ANIMATIONS);
        vTaskDelay(pdMS_TO_TICKS(2000));
    }
}
